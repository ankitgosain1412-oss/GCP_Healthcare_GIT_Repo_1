...
CREATE TABLE IF NOT EXISTS `gcp-healthcare-project-481608.gold_dataset.provider_charge_summary` (
...
# truncate table
TRUNCATE TABLE `gcp-healthcare-project-481608.gold_dataset.provider_charge_summary`;

# insert data
INSERT INTO `gcp-healthcare-project-481608.gold_dataset.provider_charge_summary`
SELECT 
    CONCAT(p.firstname, ' ', p.LastName) AS Provider_Name,
    d.Name AS Dept_Name,
    SUM(t.Amount) AS Amount
FROM `gcp-healthcare-project-481608.silver_dataset.transactions` t
LEFT JOIN `gcp-healthcare-project-481608.silver_dataset.providers` p 
    ON SPLIT(p.ProviderID, "-")[SAFE_OFFSET(1)] = t.ProviderID
LEFT JOIN `gcp-healthcare-project-481608.silver_dataset.departments` d 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `gcp-healthcare-project-481608.gold_dataset.patient_history` (
    Patient_Key STRING,
    FirstName STRING,
    LastName STRING,
    Gender STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `gcp-healthcare-project-481608.gold_dataset.patient_history`;

# INSERT DATA
INSERT INTO `gcp-healthcare-project-481608.gold_dataset.patient_history`
SELECT 
    p.Patient_Key,
    p.FirstName,
    p.LastName,
    ...
    c.PayorType
FROM `gcp-healthcare-project-481608.silver_dataset.patients` p
LEFT JOIN `gcp-healthcare-project-481608.silver_dataset.encounters` e 
    ON SPLIT(p.Patient_Key, '-')[OFFSET(0)] || '-' || SPLIT(p.Patient_Key, '-')[OFFSET(1)] = e.PatientID
LEFT JOIN `gcp-healthcare-project-481608.silver_dataset.transactions` t 
    ON SPLIT(p.Patient_Key, '-')[OFFSET(0)] || '-' || SPLIT(p.Patient_Key, '-')[OFFSET(1)] = t.PatientID
LEFT JOIN `gcp-healthcare-project-481608.silver_dataset.claims` c 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `gcp-healthcare-project-481608.gold_dataset.provider_performance` (
    ProviderID STRING,
    FirstName STRING,
    LastName STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `gcp-healthcare-project-481608.gold_dataset.provider_performance`;

# INSERT DATA
INSERT INTO `gcp-healthcare-project-481608.gold_dataset.provider_performance`
SELECT 
    pr.ProviderID,
    pr.FirstName,
    pr.LastName,
...
FROM `gcp-healthcare-project-481608.silver_dataset.providers` pr
LEFT JOIN `gcp-healthcare-project-481608.silver_dataset.encounters` e 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `gcp-healthcare-project-481608.gold_dataset.department_performance` (
    Dept_Id STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `gcp-healthcare-project-481608.gold_dataset.department_performance`;

# INSERT DATA
INSERT INTO `gcp-healthcare-project-481608.gold_dataset.department_performance`
SELECT 
    d.Dept_Id,
...
FROM `gcp-healthcare-project-481608.silver_dataset.departments` d
LEFT JOIN `gcp-healthcare-project-481608.silver_dataset.encounters` e 
